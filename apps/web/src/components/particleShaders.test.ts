...
describe('心跳（预设 13：爱心曲线 + 心跳包络 + 星空背景）', () => {
  ...
  it('内心星尘（v2 唯一保留项）：三层分桶 + 向原点收缩 s∈[0.12,0.78] + 闪烁 + 心跳呼吸', () => {
    ...
    // 星尘 = 同款曲线 → 向原点收缩（星形性保证必在心内）+ 粉/紫双色 + 独立闪烁 + 心跳呼吸
  ...
  it('v4 形状定稿：曲线主体 = 原版忠实移植（锐利霓虹心），禁再柔化/重投/减亮磨角', () => {
    // 用户定稿（「心应该是这样的」）：凹口细刺与底尖亮尖是心形特征——
    // v2 柔化套件与 v3 密度重投均被否决，全部锁死禁止回潮
    expect(heartCode.includes('ht = ht + 1.05'), '主体尖点被重投挪走').toBe(false);
    expect(heartCode.includes('dt1 = dt1 + 1.05'), '星尘尖点被重投挪走').toBe(false);
    expect(heartCode.includes('axisDamp'), '漂移抑制会磨圆尖角').toBe(false);
    // travel 无乘子（分号结尾 = 无漂移衰减）、alpha 无年轻减亮与尖点压制（原版线性衰减）
    expect(heartCode).toMatch(/float travel = HEART_V0 \* \(tau - 0\.5 \* HEART_DRAG \* tau \* tau\);/);
    expect(heartCode).toMatch(/vAlpha = bloomIn \* \(1\.0 - life\) \* \(1\.0 \+ uBass \* 0\.12\);/);
    // 发射点贴线（无 ±7px 抖动）：主体分支不得出现抖动项
    expect(heartCode).not.toMatch(/vec2\(hx, hy\) \+ vec2\(hash11\(aRand \* 23\.7\)/);
  });
