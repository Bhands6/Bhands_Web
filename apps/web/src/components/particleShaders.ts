...
//  · v2 观感（用户选向）：新增 18% 内心星尘层（向心收缩曲线采样，必在心形内部），
//    空心变实心发光体
//  · ⚠️ v2 柔化套件（漂移抑制/发射点抖动/年轻减亮/尖点减亮）与 v3 密度重投均已撤销
//    （用户 v4 否决）：凹口细刺与底尖亮尖是原版「科技祝福」的心形特征，柔化把角磨圆、
//    重投把尖挪没——曲线主体恢复原版忠实移植，只保留星尘填充
...
//  · 音频只进亮度与心跳（工作流 4.7③），不接位置量
  // ====================================================
    ...
    if (isStar) {
      ...
      // 径向外飄 = 原版忠实（v4 定稿）：尖点（凹口/底尖）粒子照常漂移——
      // 凹口细刺与底尖亮尖是原版「科技祝福」的心形特征（用户 v4 拍板），
      // ⚠️ v2 的漂移抑制/发射点抖动/年轻减亮/尖点减亮全部撤销——它们把锐利霓虹管
      // 摊成蓬松粉雾、把角磨圆（用户：「角没了」），v3 密度重投同样否决
      ...
      // alpha：显现 × 生命周期线性衰减（原版 alpha = 1 − age/duration，忠实移植）
