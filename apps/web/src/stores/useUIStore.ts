 ...
 * 超时仍未移入则自动收起。2026-09-15 定稿：面板整体对齐控制栏/搜索栏的「移开即收」——
 * 本窗口只管「打开后始终没移入」，移入后的移开由 QueuePanel.panelLeave 承担。
 */
...
interface UIState {
   ...
   * 底部「队列」按钮打开时不置位。鼠标进入面板（或左缘热区）会立即清掉该标志。
   * 2026-09-15 定稿：面板对齐控制栏/搜索栏「移开即收」——移入后的移开由
   * QueuePanel.panelLeave 收起（pinned 除外），本标志只负责「始终没移入」的情形。
   */
